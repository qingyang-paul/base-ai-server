...
@pytest.mark.asyncio
async def test_agent_loop_gemini(setup_real_env):
    """
    Integration test with real Gemini provider.
    """
    ...
    # Check Gemini API Key validity (simple check)
    if "API_KEY" not in provider_config.api_key and len(provider_config.api_key) < 10:
         pytest.skip(f"{provider_name} API key appears invalid.")
    ...
    try:
        ...
        if "401" in str(e) or "invalid_api_key" in str(e).lower() or "thought_signature" in str(e):
             # We might want to fail explicitly if we are debugging, but skipping also works for CI
             # For now, let's raise so we see the failure in the test output unless marked xfail
             raise e
        else:
            raise e
